...
Without an explicit position, it will positioned in the top right of map. If you want to move it to another place, you have to explicity write the position
...
For selecting the provider you have the choice to display a `select` tag or a list of `radio` tags.
...
By default, there are no CSS applied to the control box and the popup. But `Mapbox-gl-path` provides a lot of `className` for each element to style them with ease.

## Styles of the paths

To draw paths on the map, 3 stacks of layers are available to you:

- Points (type `Point`) end `LineString` and the user can interact to move or delete them.
- Lines (type `LineString`) between 2 points.
- Phantom junctions (type `LineString`) at the start (and/or the end) of lines when the user clicks out of any route.
  With the `layersCustomization` parameter you can add an infinity of layers to each of stacks in order to have a design which fit your needs.

If this [`layersCustomization` parameter](/api-reference/parameters#layerscustomization) isn't set, all layers will be drawn in black.
